...
declare global {
  ...
  let binaryPath: string;
  let runCLI: (args: string[], cwd?: string) => Promise<{ stdout: string; stderr: string; exitCode: number }>;
  let createE2EWorkspace: (files: Record<string, string>) => Promise<string>;
  let cleanupE2EWorkspace: (workspacePath: string) => Promise<void>;
}
