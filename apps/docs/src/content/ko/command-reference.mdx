...
| `pcu check`     | `pcu chk`, `pcu -c` | 오래된 카탈로그 종속성 확인       |
| `pcu update`    | `pcu u`, `pcu -u`   | 카탈로그 종속성 업데이트          |
| `pcu analyze`   | `pcu a`, `pcu -a`   | 종속성 업데이트의 영향 분석       |
| `pcu workspace` | `pcu w`, `pcu -s`   | 작업공간 정보 및 유효성 검사 표시 |
| `pcu theme`     | `pcu t`, `pcu -t`   | 색상 테마 및 UI 설정 구성         |
