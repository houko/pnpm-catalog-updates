...
| `pcu check`     | `pcu chk`, `pcu -c` | 古いカタログ依存関係をチェック         |
| `pcu update`    | `pcu u`, `pcu -u`   | カタログ依存関係を更新                 |
| `pcu analyze`   | `pcu a`, `pcu -a`   | 依存関係更新の影響を分析               |
| `pcu workspace` | `pcu w`, `pcu -s`   | ワークスペース情報と検証を表示         |
| `pcu theme`     | `pcu t`, `pcu -t`   | カラーテーマと UI 設定を構成           |
